    ...
    for group in allergens:
        if group.get('id') == 'allergens':
            for attribute in group.get('attributes', []):
                if "Ne contient pas" in attribute.get('title', ''):
                    status = Fore.WHITE + "Ne contient pas" + Style.RESET_ALL
                    title = Fore.WHITE + attribute.get('title', 'N/A') + Style.RESET_ALL
                else:
                    status = Fore.RED + "Contient" + Style.RESET_ALL
                    title = Fore.RED + attribute.get('title', 'N/A') + Style.RESET_ALL
...
if __name__ == "__main__":
    ...
    print("Scannez le code bar d'un produit:")  # Prompt message for user
    barcode = input()  # Input function to capture the barcode
    print(f"Obtention des détails pour {barcode}...")  # Display the barcode

    product = get_product_details(barcode)  # Fetch product details

    if product:
        print_product_details(product)  # Print product details if found
    else:
        print("Produit non trouvé.")  # Print message if product not found
